// ============================================================
// Quiz Page — Interactive Quiz (English)
// ============================================================
...
export function QuizPage(): HTMLElement {
  ...
  main.innerHTML = `
    <style>
      ...
      .quiz-grid { display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:1.25rem;animation:slideUp 0.5s 0.1s ease both; }

      .quiz-card {
        background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);
        border-radius:20px;padding:1.5rem;backdrop-filter:blur(20px);cursor:pointer;
        transition:all 0.3s;position:relative;overflow:hidden;
      }
      .quiz-card:hover { border-color:rgba(56,189,248,0.3);transform:translateY(-4px);box-shadow:0 15px 40px rgba(0,0,0,0.3); }

      .quiz-module-badge {
        display:inline-block;font-size:0.68rem;font-weight:700;
        padding:0.25rem 0.7rem;border-radius:20px;letter-spacing:0.06em;margin-bottom:0.75rem;
      ...
      .quiz-meta { display:flex;gap:0.75rem;flex-wrap:wrap;margin-top:0.75rem;padding-top:0.75rem;border-top:1px solid rgba(255,255,255,0.05); }
      .quiz-meta-item { font-size:0.75rem;color:#64748b;display:flex;align-items:center;gap:0.3rem; }

      .diff-badge { display:inline-block;font-size:0.7rem;font-weight:700;padding:0.2rem 0.6rem;border-radius:8px; }
      ...
      .quiz-header-bar { display:flex;align-items:center;justify-content:space-between;margin-bottom:2rem;flex-wrap:wrap;gap:1rem; }
      ...
      .question-card {
        ...
        margin-bottom:1.5rem;animation:popIn 0.4s cubic-bezier(0.4,0,0.2,1);
      }
      ...
      .option-btn {
        ...
        background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);
        border-radius:14px;color:#94a3b8;font-size:0.9rem;font-weight:500;cursor:pointer;
        transition:all 0.25s;position:relative;overflow:hidden;
      }
      .option-btn::before {
        content:'';position:absolute;left:0;top:0;bottom:0;width:3px;
        background:linear-gradient(135deg,#38bdf8,#818cf8);opacity:0;transition:opacity 0.25s;border-radius:3px 0 0 3px;
      }
      .option-btn:hover:not(:disabled) { border-color:rgba(56,189,248,0.35);background:rgba(56,189,248,0.06);color:#e2e8f0; }
      ...
      .btn-next {
        padding:0.8rem 2rem;background:linear-gradient(135deg,#0ea5e9,#6366f1);
      ...
      .btn-quit { padding:0.8rem 1.5rem;background:transparent;border:1px solid rgba(255,255,255,0.1);color:#64748b;border-radius:12px;font-size:0.85rem;cursor:pointer;transition:all 0.25s; }
      ...
      .result-screen {
        ...
        border-radius:24px;backdrop-filter:blur(20px);animation:popIn 0.5s cubic-bezier(0.4,0,0.2,1);
      }
      ...
      .btn-retry {
        padding:0.85rem 2rem;background:linear-gradient(135deg,#0ea5e9,#6366f1);
        color:white;border:none;border-radius:12px;font-size:0.9rem;font-weight:700;cursor:pointer;
        transition:all 0.3s;box-shadow:0 4px 20px rgba(14,165,233,0.3);
      }
      .btn-retry:hover { transform:translateY(-2px);box-shadow:0 8px 30px rgba(14,165,233,0.5); }
      .btn-back {
        padding:0.85rem 2rem;background:transparent;border:1px solid rgba(255,255,255,0.12);
        color:#94a3b8;border-radius:12px;font-size:0.9rem;cursor:pointer;transition:all 0.25s;
      }
      .btn-back:hover { border-color:rgba(56,189,248,0.3);color:#38bdf8; }

      .sk-card { height:160px;border-radius:20px;background:linear-gradient(90deg,rgba(255,255,255,0.04) 25%,rgba(255,255,255,0.08) 50%,rgba(255,255,255,0.04) 75%);background-size:200% 100%;animation:shimmer 1.5s infinite; }
      .no-results { text-align:center;padding:3rem;color:#475569; }
    </style>

    <div class="quiz-page">
      <div class="page-header" id="quiz-list-header">
        <h1 class="page-title">📝 Adaptive Quizzes</h1>
        <p class="page-sub">Test your knowledge and improve your mastery</p>
      </div>
      ...
      <div class="result-screen" id="result-screen">
        ...
        <div class="result-label" id="result-label">Final score</div>
        <div class="result-actions">
          <button class="btn-retry" id="btn-retry">🔄 Try Again</button>
          <button class="btn-back" id="btn-back-result">← Back to Quizzes</button>
        </div>
      </div>
    </div>
  `
  ...
  const showList = () => {
    ;(quizGrid as HTMLElement).style.display = 'grid'
    ;(quizListHeader as HTMLElement).style.display = 'block'
    quizActive.classList.remove('visible'); resultScreen.classList.remove('visible')
    currentQuiz = null; currentQ = 0; answers = []
  }
  ...
  const renderQuestion = () => {
    ...
    btnNext.disabled = true; selectedAnswer = null
  }

  const startQuiz = (quiz: Quiz) => {
    currentQuiz = quiz; currentQ = 0; answers = []; selectedAnswer = null
    ;(quizGrid as HTMLElement).style.display = 'none'
    ;(quizListHeader as HTMLElement).style.display = 'none'
    quizActive.classList.add('visible'); resultScreen.classList.remove('visible')
    ;(main.querySelector('#active-quiz-title') as HTMLElement).textContent = quiz.titre
    renderQuestion()
  }

  const diffLabel = (d: string) => d === 'facile' ? 'Easy' : d === 'difficile' ? 'Hard' : 'Medium'
  ...
  btnNext.addEventListener('click', () => {
    ...
    setTimeout(() => {
      ...
      if (currentQ < currentQuiz!.questions.length) { renderQuestion() }
        ...
        quizActive.classList.remove('visible'); resultScreen.classList.add('visible')
        ...
        ;(main.querySelector('#result-label') as HTMLElement).textContent =
          `${correct}/${currentQuiz!.questions.length} correct — ${score >= 70 ? 'Concept mastered!' : 'Keep practicing'}`

        if (user) {
          api.submitQuiz(currentQuiz!.id, { etudiant_id: user.id, score, reponses: answers, temps_reponse: 0 }).catch(() => {})
        }
      }
    }, 800)
  })
  ...
  main.querySelector('#btn-retry')?.addEventListener('click', () => { if (currentQuiz) startQuiz(currentQuiz) })

  api.getQuizList().then((quizzes: Quiz[]) => {
    if (quizzes.length === 0) {
      quizGrid.innerHTML = '<div class="no-results" style="grid-column:1/-1;"><div style="font-size:2.5rem;margin-bottom:1rem">📭</div><p>No quizzes available yet.</p></div>'
      return
    }
      ...
      return `
        <div class="quiz-card" data-id="${q.id}">
          ...
          <div class="quiz-meta">
            ...
            <span class="diff-badge ${diffClass}">${diffLabel(q.difficulte)}</span>
          </div>
        </div>
      `
    }).join('')
    ...
    quizGrid.innerHTML = `<div class="no-results" style="grid-column:1/-1;"><div style="font-size:2.5rem;margin-bottom:1rem">🔌</div><p>Backend not connected. Start uvicorn and reload.</p></div>`
  })
