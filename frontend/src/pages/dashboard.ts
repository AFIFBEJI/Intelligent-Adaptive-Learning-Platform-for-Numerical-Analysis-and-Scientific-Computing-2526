// ============================================================
// Dashboard — 3D Cards + Animated Progress
// ============================================================
...
export function DashboardPage(): HTMLElement {
  ...
  main.innerHTML = `
    <style>
      ...
      @keyframes ringFill { from{stroke-dashoffset:440} to{stroke-dashoffset:var(--offset)} }
    ...
    <div class="dashboard">
      ...
      <div class="welcome-bar">
        <div>
          <div class="welcome-title">Bonjour, <span>${user?.nom_complet?.split(' ')[0] || 'Étudiant'}</span> 👋</div>
          <div class="welcome-sub">Votre parcours adaptatif en analyse numérique</div>
        </div>
        <div class="niveau-pill">⚡ ${user?.niveau_actuel || 'Intermédiaire'}</div>
      </div>
      ...
      <div class="main-grid">
        <div class="panel">
          <div class="panel-title">🔁 À améliorer</div>
          <div id="improve-list">
        ...
        <div class="panel">
          <div class="panel-title">⭐ Recommandés</div>
  ...
  if (user) {
    api.getLearningPath(user.id).then((path: LearningPath) => {
      ...
      const statsData = [
        { icon:'📚', cls:'si-blue', num: total_concepts, label:'CONCEPTS TOTAL' },
        { icon:'✅', cls:'si-green', num: mastered, label:'MAÎTRISÉS (≥70%)' },
        { icon:'🔥', cls:'si-orange', num: in_progress, label:'EN COURS' },
        { icon:'⭐', cls:'si-purple', num: toDiscover, label:'À DÉCOUVRIR' },
      ]
      ...
      if (path.concepts_to_improve.length === 0) {
        improveList.innerHTML = `<div class="empty-state"><div class="empty-icon">🎉</div><p>Aucun concept en retard !</p></div>`
      } else {
        improveList.innerHTML = path.concepts_to_improve.map(c => `
          <div class="concept-row">
            <div class="concept-name">${c.name}</div>
            <div class="concept-bar-wrap">
              <div class="concept-bar bar-orange" style="width:${c.mastery}%"></div>
            </div>
      ...
      if (path.next_recommended.length === 0) {
        recommendList.innerHTML = `<div class="empty-state"><div class="empty-icon">📖</div><p>Complétez d'abord les concepts en cours</p></div>`
      } else {
        recommendList.innerHTML = path.next_recommended.map(c => `
          <div class="rec-card">
            <div class="rec-dot"></div>
            <div class="rec-name">${c.name}</div>
            <span class="rec-badge">Prêt</span>
          </div>
        `).join('')
      }
    }).catch(() => {
      main.querySelector('#improve-list')!.innerHTML = `<div class="empty-state"><div class="empty-icon">🎯</div><p>Faites votre premier quiz pour voir votre progression</p></div>`
      main.querySelector('#recommend-list')!.innerHTML = `<div class="empty-state"><div class="empty-icon">🧠</div><p>Données non disponibles</p></div>`
    })
  }
